...
function emailWrapper(title: string, body: string, choirName: string): string {
  return `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 560px; margin: 0 auto; padding: 20px;">
      <div style="background: #ffffff; border: 1px solid #d9e2ec; box-shadow: 0 20px 48px rgba(15, 23, 42, 0.08); border-radius: 16px; padding: 32px; color: #172033;">
        ...
        <div style="background: #f8fafc; border: 1px solid #e4e7ec; border-radius: 12px; padding: 24px; margin-bottom: 16px;">
          <h3 style="color: #172033; margin: 0 0 16px 0; font-size: 18px;">${title}</h3>
          ${body}
        </div>
        <div style="text-align: center; margin-top: 20px; padding-top: 16px; border-top: 1px solid #e4e7ec;">
          <p style="color: #667085; font-size: 14px; margin: 0 0 4px 0; font-style: italic;">Yours faithfully,</p>
...
export async function notifyLeaveRequestCreated(
  ...
  const html = emailWrapper("New Leave Request", `
    ...
    <table style="width: 100%; border-collapse: collapse;">
      <tr><td style="color: #667085; padding: 4px 8px 4px 0; white-space: nowrap;">From:</td><td style="color: #172033; padding: 4px 0;">${new Date(startDate).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}</td></tr>
      <tr><td style="color: #667085; padding: 4px 8px 4px 0; white-space: nowrap;">To:</td><td style="color: #172033; padding: 4px 0;">${new Date(endDate).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}</td></tr>
      <tr><td style="color: #667085; padding: 4px 8px 4px 0; white-space: nowrap;">Reason:</td><td style="color: #172033; padding: 4px 0;">${reason}</td></tr>
    </table>
    <p style="color: #d4af37; margin: 16px 0 0 0; font-size: 14px;">Please log in to the admin portal to review and vote on this request.</p>
  `, settings.choirName);
...
export async function notifyLeaveRequestDecision(
  ...
  const html = emailWrapper(`Leave Request ${statusText}`, `
    <p style="color: #344054; margin: 0 0 12px 0;">Hi <strong>${memberName}</strong>,</p>
    <p style="color: #344054; margin: 0 0 16px 0;">Your leave request has been <span style="color: ${statusColor}; font-weight: bold;">${statusText.toLowerCase()}</span>.</p>
    <table style="width: 100%; border-collapse: collapse;">
      <tr><td style="color: #667085; padding: 4px 8px 4px 0; white-space: nowrap;">From:</td><td style="color: #172033; padding: 4px 0;">${new Date(startDate).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}</td></tr>
      <tr><td style="color: #667085; padding: 4px 8px 4px 0; white-space: nowrap;">To:</td><td style="color: #172033; padding: 4px 0;">${new Date(endDate).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric", year: "numeric" })}</td></tr>
...
export async function notifyUnlockRequestCreated(
  ...
  const html = emailWrapper("Month Unlock Request", `
    <p style="color: #344054; margin: 0 0 12px 0;"><strong>${requestedBy}</strong> (${requestedByRole}) has requested to unlock a locked month.</p>
    <table style="width: 100%; border-collapse: collapse;">
      <tr><td style="color: #667085; padding: 4px 8px 4px 0; white-space: nowrap;">Month:</td><td style="color: #172033; padding: 4px 0;">${monthName} ${year}</td></tr>
      <tr><td style="color: #667085; padding: 4px 8px 4px 0; white-space: nowrap;">Type:</td><td style="color: #172033; padding: 4px 0;">${typeLabel}</td></tr>
      <tr><td style="color: #667085; padding: 4px 8px 4px 0; white-space: nowrap;">Reason:</td><td style="color: #172033; padding: 4px 0;">${reason}</td></tr>
    </table>
    <p style="color: #d4af37; margin: 16px 0 0 0; font-size: 14px;">Please log in to the admin portal Settings to approve or deny this request.</p>
  `, settings.choirName);
...
export async function notifyUnlockRequestDecision(
  ...
  const html = emailWrapper(`Unlock Request ${statusText}`, `
    <p style="color: #344054; margin: 0 0 12px 0;">Hi <strong>${requesterName}</strong>,</p>
    <p style="color: #344054; margin: 0 0 16px 0;">Your unlock request for <strong>${monthName} ${year}</strong> has been <span style="color: ${statusColor}; font-weight: bold;">${statusText.toLowerCase()}</span> by ${reviewerName}.</p>
...
export async function notifyContributionRecorded(
  ...
  const html = emailWrapper("Contribution Receipt", `
    ...
    <table style="width: 100%; border-collapse: collapse;">
      ${category === "monthly" ? `<tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Month:</td><td style="color: #172033; padding: 6px 0; border-bottom: 1px solid #e4e7ec; text-align: right;">${monthName} ${year}</td></tr>` : ""}
      ${typeName ? `<tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Type:</td><td style="color: #172033; padding: 6px 0; border-bottom: 1px solid #e4e7ec; text-align: right;">${typeName}</td></tr>` : ""}
      <tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Amount Paid:</td><td style="color: #22c55e; padding: 6px 0; border-bottom: 1px solid #e4e7ec; text-align: right; font-weight: bold; font-size: 16px;">${formatCurrencyEmail(amount)}</td></tr>
      ${expectedAmount > 0 ? `<tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Expected:</td><td style="color: #172033; padding: 6px 0; border-bottom: 1px solid #e4e7ec; text-align: right;">${formatCurrencyEmail(expectedAmount)}</td></tr>` : ""}
      ${!isFullyPaid && remaining > 0 ? `<tr><td style="color: #667085; padding: 6px 8px 6px 0;">Remaining:</td><td style="color: #eab308; padding: 6px 0; text-align: right; font-weight: bold;">${formatCurrencyEmail(remaining)}</td></tr>` : ""}
    </table>
    ${isFullyPaid
      ? `<p style="color: #22c55e; margin: 16px 0 0 0; font-size: 14px; text-align: center;">Thank you for your full payment!</p>`
...
export async function notifyAnnouncementPosted(
  ...
  const html = emailWrapper("Announcement", `
    ...
    <h3 style="color: #172033; margin: 0 0 12px 0;">${title}</h3>
    <div style="color: #344054; line-height: 1.6; white-space: pre-line;">${content}</div>
    <p style="color: #d4af37; margin: 16px 0 0 0; font-size: 13px;">Visit the member portal for more details.</p>
  `, settings.choirName);
...
export async function notifyEventCreated(
  ...
  const html = emailWrapper("New Event", `
    ...
    <table style="width: 100%; border-collapse: collapse;">
      <tr><td style="color: #667085; padding: 6px 8px 6px 0; white-space: nowrap;">Date:</td><td style="color: #172033; padding: 6px 0;">${formattedDate}</td></tr>
      <tr><td style="color: #667085; padding: 6px 8px 6px 0; white-space: nowrap;">Time:</td><td style="color: #172033; padding: 6px 0;">${eventTime}</td></tr>
      <tr><td style="color: #667085; padding: 6px 8px 6px 0; white-space: nowrap;">Location:</td><td style="color: #172033; padding: 6px 0;">${eventLocation}</td></tr>
      ${isFree !== undefined ? `<tr><td style="color: #667085; padding: 6px 8px 6px 0; white-space: nowrap;">Admission:</td><td style="color: ${isFree ? "#22c55e" : "#d4af37"}; padding: 6px 0;">${isFree ? "Free" : "Ticketed"}</td></tr>` : ""}
    </table>
    ${description ? `<p style="color: #344054; margin: 16px 0 0 0; line-height: 1.5;">${description.substring(0, 300)}${description.length > 300 ? "..." : ""}</p>` : ""}
    <p style="color: #d4af37; margin: 16px 0 0 0; font-size: 13px;">Mark your calendar! Visit the portal for more details.</p>
  `, settings.choirName);
...
export async function notifyDisciplinaryAction(
  ...
  const html = emailWrapper(isPositive ? "Commendation" : "Disciplinary Notice", `
    <p style="color: #344054; margin: 0 0 12px 0;">Dear <strong>${memberName}</strong>,</p>
    ${isPositive
      ? `<p style="color: #22c55e; margin: 0 0 16px 0;">Congratulations! You have received a commendation from the choir leadership.</p>`
      : `<p style="color: #344054; margin: 0 0 16px 0;">This is to inform you of a disciplinary action that has been recorded.</p>`
    }
    <table style="width: 100%; border-collapse: collapse;">
      <tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Type:</td><td style="color: #172033; padding: 6px 0; border-bottom: 1px solid #e4e7ec;">${typeLabel}</td></tr>
      ${!isPositive ? `<tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Severity:</td><td style="color: ${severityColor}; padding: 6px 0; border-bottom: 1px solid #e4e7ec; font-weight: bold;">${severity.charAt(0).toUpperCase() + severity.slice(1)}</td></tr>` : ""}
      <tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Reason:</td><td style="color: #172033; padding: 6px 0; border-bottom: 1px solid #e4e7ec;">${reason}</td></tr>
      ${actionTaken ? `<tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Action:</td><td style="color: #172033; padding: 6px 0; border-bottom: 1px solid #e4e7ec;">${actionTaken}</td></tr>` : ""}
      ${expiryDate ? `<tr><td style="color: #667085; padding: 6px 8px 6px 0;">Expires:</td><td style="color: #172033; padding: 6px 0;">${new Date(expiryDate).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</td></tr>` : ""}
    </table>
    ${isPositive
...
export async function notifyDisciplinaryResolved(
  ...
  const html = emailWrapper("Record Resolved", `
    ...
    <table style="width: 100%; border-collapse: collapse;">
      <tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Record Type:</td><td style="color: #172033; padding: 6px 0; border-bottom: 1px solid #e4e7ec;">${typeLabel}</td></tr>
      <tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Resolution:</td><td style="color: #22c55e; padding: 6px 0; border-bottom: 1px solid #e4e7ec;">${resolution}</td></tr>
      <tr><td style="color: #667085; padding: 6px 8px 6px 0;">Resolved By:</td><td style="color: #172033; padding: 6px 0;">${resolvedBy}</td></tr>
    </table>
    <p style="color: #22c55e; margin: 16px 0 0 0; font-size: 14px;">This matter is now closed. Thank you for your cooperation.</p>
  `, settings.choirName);
...
export async function notifyMemberStatusChanged(
  ...
  const html = emailWrapper("Membership Status Update", `
    ...
    <p style="color: #344054; margin: 0 0 16px 0;">Your membership status has been updated.</p>
    <table style="width: 100%; border-collapse: collapse;">
      <tr><td style="color: #667085; padding: 6px 8px 6px 0; border-bottom: 1px solid #e4e7ec;">Previous Status:</td><td style="color: #172033; padding: 6px 0; border-bottom: 1px solid #e4e7ec;">${oldStatus}</td></tr>
      <tr><td style="color: #667085; padding: 6px 8px 6px 0;">New Status:</td><td style="color: ${statusColor}; padding: 6px 0; font-weight: bold;">${newStatus}</td></tr>
    </table>
    ${newStatus === "Active"
      ? `<p style="color: #22c55e; margin: 16px 0 0 0; font-size: 14px;">Welcome! You now have full access to choir activities and the member portal.</p>`
      : newStatus === "Inactive"
...
export async function notifySurveyPublished(
  ...
  const html = emailWrapper("New Survey", `
    ...
    ${surveyDescription ? `<p style="color: #344054; margin: 0 0 16px 0; line-height: 1.5;">${surveyDescription}</p>` : ""}
    <p style="color: #344054; margin: 0 0 8px 0;">A new survey is available for you to complete. Your feedback is important to us!</p>
    <p style="color: #d4af37; margin: 16px 0 0 0; font-size: 13px;">Visit the member portal to submit your response.</p>
  `, settings.choirName);
...
export async function notifyMeetingMinutesApproved(
  ...
  const html = emailWrapper("Meeting Minutes", `
    ...
    <table style="width: 100%; border-collapse: collapse;">
      <tr><td style="color: #667085; padding: 6px 8px 6px 0;">Date:</td><td style="color: #172033; padding: 6px 0;">${formattedDate}</td></tr>
      <tr><td style="color: #667085; padding: 6px 8px 6px 0;">Approved By:</td><td style="color: #172033; padding: 6px 0;">${approvedBy}</td></tr>
    </table>
    <p style="color: #344054; margin: 16px 0 0 0;">The meeting minutes have been approved and are now available for leadership review in the admin portal.</p>
  `, settings.choirName);
